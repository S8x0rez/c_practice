...
//  }
	...
	//ビットマップファイルの画像データ部読み込み
	 ...
	 for (k = 0; k < height; k++) {
		 for (j = 0; j < width; j++) {
			 img_out->Y[j + k * width] = img->Y[j + k * width];
			 img_out->U[j + k * width] = img->U[j + k * width];
			 img_out->V[j + k * width] = img->V[j + k * width];
		 }
	 }
	 ...
	 for (k = 1; k < height - 1; k++) {        // 画像の上下のふち1列は計算しない
		 for (j = 1; j < width - 1; j++) {     // 画像の左右のふち1列は計算しない
			 ...
			 value = -img->Y[j + (k - 1)*width]
				 - img->Y[j - 1 + k * width] + 5 * img->Y[j + k * width] - img->Y[j + 1 + k * width]
				 - img->Y[j + (k + 1)*width];
			 value = abs(value);
	 ...
	 for (k = 1; k < height - 1; k++) {        // 画像の上下のふち1列は計算しない
		 for (j = 1; j < width - 1; j++) {     // 画像の左右のふち1列は計算しない
			 ...
			 /* Sobel filter (horizontal edge detection) */
			 value = -img->U[j + (k - 1)*width]
				 - img->U[j - 1 + k * width] + 5 * img->U[j + k * width] - img->U[j + 1 + k * width]
				 - img->U[j + (k + 1)*width];
			 value = abs(value);

			 img_out->U[j + k * width] = rounding_integer(value);  // 整数値を0～255にクリップする関数
		 }
	 }
	 for (k = 1; k < height - 1; k++) {        // 画像の上下のふち1列は計算しない
		 for (j = 1; j < width - 1; j++) {     // 画像の左右のふち1列は計算しない
			 ...
			 /* Sobel filter (horizontal edge detection) */
			 value = -img->V[j + (k - 1)*width]
				 - img->V[j - 1 + k * width] + 5 * img->V[j + k * width] - img->V[j + 1 + k * width]
				 - img->V[j + (k + 1)*width];
			 value = abs(value);

			 img_out->V[j + k * width] = rounding_integer(value);  // 整数値を0～255にクリップする関数
		 }
	 }
